...
namespace QuestScript.Interpreter.Exceptions
{
    ...
    {
        public FailedToInferTypeException(ParserRuleContext ctx, ParserRuleContext invalidExpressionContext, string additionalMessage) : 
            base(ctx, $"Failed to infer the type of expression '{invalidExpressionContext.GetText()}'. {additionalMessage}")
        {
        }
